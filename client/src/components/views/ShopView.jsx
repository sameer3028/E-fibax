...
export function ShopView({
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchCategory = selectedCategory === 'all' || isProductInCategory(p, selectedCategory);
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-charcoal-muted mt-1 max-w-xl">
              {activeConcernObj
                ...
                : 'Showing verified proprietary herbal medicines manufactured under strict GMP quality standards.'}
            </p>
          </div>
          ...
          <aside className="hidden lg:block bg-white p-6 rounded-3xl border border-sand-border shadow-subtle space-y-6 sticky top-24">
            ...
            <div className="pt-4 border-t border-sand-border">
              ...
              <div className="space-y-1">
                ...
                {CATEGORIES.map((cat) => {
                  const catCount = products.filter((p) => isProductInCategory(p, cat.id) || isProductInCategory(p, cat.slug)).length;

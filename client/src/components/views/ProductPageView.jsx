...
export function ProductPageView({
  ...
  // Configurable Treatment Course / Value Pack configuration
  const tcConfig = product?.treatmentCourseConfig;
  const isTcEnabled = !!(
    tcConfig &&
    tcConfig.enabled &&
    Array.isArray(tcConfig.packs) &&
    tcConfig.packs.length > 0
  );

  const activePacks = isTcEnabled ? tcConfig.packs : [];
  const currentPack = isTcEnabled ? (activePacks[selectedPackIndex] || activePacks[0]) : null;

  const currentPrice = isTcEnabled && currentPack ? Number(currentPack.price) : Number(product.salePrice || 0);
  const currentQuantityInPack = isTcEnabled && currentPack ? (Number(currentPack.quantity) || 1) : 1;
  const unitPrice = isTcEnabled && currentPack ? Math.round(currentPrice / currentQuantityInPack) : Number(product.salePrice || 0);

  const baseMrpUnit = Number(product.mrp) || Math.round((Number(product.salePrice) || 0) * 1.25);
  const regularMrpForPack = baseMrpUnit * currentQuantityInPack;
  const totalSavings = Math.max(0, regularMrpForPack - currentPrice);

  // Handle Add To Cart
  const handleAddToCart = () => {
    if (isOutOfStock) return;
    if (isTcEnabled && currentPack) {
      addToCart(
        {
          ...product,
          salePrice: Number(currentPack.price),
          mrp: regularMrpForPack,
          selectedPackName: currentPack.name,
          selectedPackQty: currentQuantityInPack,
          selectedPackPrice: Number(currentPack.price),
          selectedPackDuration: currentPack.duration || ''
        },
        quantity
      );
    } else {
      addToCart(
        {
          ...
          mrp: Number(product.mrp || baseMrpUnit)
        },
        quantity
      );
    }
          ...
          <div className="lg:col-span-7 flex flex-col justify-between space-y-6">
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {isOutOfStock && (
                  <span className="text-xs font-extrabold text-white bg-charcoal px-3 py-0.5 rounded-full uppercase tracking-wider shadow-xs">
                    OUT OF STOCK
                  </span>
                )}
                <span className="text-xs text-charcoal-muted font-mono font-semibold ml-auto">
              ...
              <div className="pt-2 pb-3 border-y border-sand-border/80 flex flex-wrap items-baseline gap-3">
                <div className="flex items-baseline gap-2">
                  ...
                  {regularMrpForPack > currentPrice && (
                ...
                {totalSavings > 0 && regularMrpForPack > 0 && (
              ...
              {/* Configurable Treatment Course / Value Pack Selector */}
              {isTcEnabled && (
                <div className="space-y-2.5 pt-1">
                  <div className="flex items-center justify-between text-xs font-bold text-forest flex-wrap gap-1">
                    <span className="uppercase tracking-wider">{tcConfig.heading || 'SELECT TREATMENT COURSE / VALUE PACK:'}</span>
                    {tcConfig.recommendationText && tcConfig.recommendationText.trim() !== '' && (
                      <span className="text-brand font-semibold">{tcConfig.recommendationText.trim()}</span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {activePacks.map((pack, idx) => {
                      ...
                      const pQty = Number(pack.quantity) || 1;
                      const pPrice = Number(pack.price) || 0;
                      const pUnitPrice = Math.round(pPrice / pQty);
                      const promoLabel = pack.promotionalLabel || (pack.discountPercent ? `${pack.discountPercent}% OFF` : '');
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-[10px] text-charcoal-muted mt-1 flex items-center justify-between">
                            <span>{formatPrice(pUnitPrice)} / unit</span>
                            {pack.duration ? <span className="font-medium text-forest/80">{pack.duration}</span> : null}
                          </div>
                        </button>
                      );
                    })}
              ...
              <div className="space-y-3 pt-3">
                {isOutOfStock ? (
                  <div className="pt-1">
                    <button
                      disabled
                      className="w-full py-4 px-6 rounded-2xl bg-sand text-charcoal-muted text-sm font-extrabold tracking-wide uppercase cursor-not-allowed flex items-center justify-center gap-2 border border-sand-border shadow-xs"
                    >
                      <AlertCircle className="h-5 w-5 text-charcoal-subtle" />
                      <span>Out of Stock</span>
                    </button>
                  </div>
                ) : (
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md border-t border-sand-border p-3 z-40 shadow-2xl flex items-center justify-between gap-4 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-baseline gap-2">
              <span className="font-bold text-forest text-sm">{formatPrice(currentPrice)}</span>
              {regularMrpForPack > currentPrice && <span className="text-[11px] text-charcoal-muted line-through">{formatPrice(regularMrpForPack)}</span>}
              {isTcEnabled && currentPack && <span className="text-[10px] text-red-600 font-bold hidden sm:inline">({currentPack.name})</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {isOutOfStock ? (
            <button
              disabled
              className="px-5 py-2.5 rounded-xl bg-sand text-charcoal-muted text-xs font-bold uppercase tracking-wide cursor-not-allowed flex items-center gap-1.5 border border-sand-border"
            >

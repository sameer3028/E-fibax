...
export function About({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    const fetchActiveBanner = async () => {
      try {
        const res = await apiRequest('/api/website/about-banners/active');
        if (isMounted && res.success && res.data) {
          setActiveBanner(res.data);
        }
      } catch (err) {
        // Fallback to null (uses default content)
      }
    };
    fetchActiveBanner();
  ...
  const badgeText = activeBanner?.badgeText || 'Authentic Ayurvedic Heritage';
  const heading = activeBanner?.heading || 'Pioneering Pure Ayurvedic Healthcare & Natural Wellness';
  const description = activeBanner?.description || 'Fibax Ayurveda is a premier herbal wellness brand dedicated to formulating authentic, research-driven botanical remedies. Backed by WHO-GMP certified infrastructure, we deliver pure, chemical-free healthcare solutions directly to households across India.';
  const primaryBtnText = activeBanner?.primaryBtnText || 'Browse 250+ Formulations';
  const primaryBtnUrl = activeBanner?.primaryBtnUrl || 'products';
  const secondaryBtnText = activeBanner?.secondaryBtnText || 'Contact Us';
  const secondaryBtnUrl = activeBanner?.secondaryBtnUrl || 'contact';
  const trustPoints = Array.isArray(activeBanner?.trustPoints) && activeBanner.trustPoints.length > 0
    ? activeBanner.trustPoints
    : ['Pure & Natural Ingredients', 'WHO-GMP Certified', 'Safe & Effective Formulations', 'Made for Healthier India'];
  const desktopImage = activeBanner?.desktopImage || '';
  const mobileImage = activeBanner?.mobileImage || desktopImage;
  return (
    <div className="w-full bg-white">
      {/* 1. Page Hero Banner (Dynamic from Admin Portal) */}
      <section className="relative py-16 sm:py-20 bg-forest-deep text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            {/* Content Column */}
            <div className={`${(desktopImage || mobileImage) ? 'lg:col-span-7' : 'lg:col-span-12'} text-center sm:text-left`}>
              ...
              {description && (
                <p className="mt-4 text-sm sm:text-base text-sand-warm/80 leading-relaxed max-w-3xl">
                  {description}
                </p>
              )}
              ...
              <div className="mt-8 flex flex-wrap gap-4 justify-center sm:justify-start">
                {primaryBtnText && (
                  <button
                    onClick={() => onNavigate && onNavigate(primaryBtnUrl || 'products')}
                    className="px-6 py-3.5 rounded-xl bg-brand hover:bg-brand-hover text-white font-bold text-sm shadow-orange-glow transition-all flex items-center gap-2"
                  >
                ...
                {secondaryBtnText && (
                  <button
                    onClick={() => onNavigate && onNavigate(secondaryBtnUrl || 'contact')}
              ...
              {trustPoints && trustPoints.length > 0 && (
                <div className="mt-8 pt-6 border-t border-white/15 grid grid-cols-2 sm:grid-cols-4 gap-3 text-left">
                  {trustPoints.map((tp, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs font-semibold text-sand-warm/90">
            ...
            {/* Optional Banner Image Column */}
            {(desktopImage || mobileImage) && (
              <div className="lg:col-span-5 flex justify-center">
                <div className="w-full max-w-md lg:max-w-none rounded-3xl overflow-hidden border-2 border-white/20 shadow-2xl bg-sand/10 p-2">
                  <picture>
